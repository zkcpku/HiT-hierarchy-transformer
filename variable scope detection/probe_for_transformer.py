...
def get_example(item):
    ...
    if hasattr(my_config, 'remove_comment') and my_config.remove_comment:
        new_code_tokens = [code_tokens[i] for i in range(len(code_tokens)) if not path_tokens[i].endswith('comment') and not path_tokens[i].endswith('\n')]
        new_path_tokens = [path_tokens[i] for i in range(len(path_tokens)) if not path_tokens[i].endswith('comment') and not path_tokens[i].endswith('\n')]
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, item):
        return torch.tensor(self.examples[item].input_ids),torch.tensor(self.examples[item].label),self.get_edge_labels(self.examples[item].input_ids,self.examples[item].path_tokens)
...
def train(args, train_dataset, model, code_vocab, path_vocab, pool):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        for step, batch in enumerate(bar):
            ...
            edge_labels = batch[2].to(args.device)
            model.train()
            loss,logits = model(inputs,edge_labels)
...
def evaluate(args, model, code_vocab, path_vocab, prefix="", pool=None, eval_when_training=False):
    ...
    for batch in eval_dataloader:
        ...
        edge_labels = batch[2].to(args.device)
        with torch.no_grad():
            lm_loss,logit = model(inputs,edge_labels)
...
def main():
    ...
    if args.model_type == 'transformer':
        encoder=TransformerClf(args.seq_vocab_size, args.cls_num, args.emb_size, args.nhead,
        ...
        model = ProbeModel(encoder, args.emb_size)
        model.to(args.device)
    else:
    ...
    #     output_dir = os.path.join(args.load_model_path, '{}'.format(checkpoint_prefix))  
