...
class TiTClf(nn.Module):
    def __init__(self, ntokens, ncls, emsizes, nhead, nhid, nlayers, path_nlayers=2, dropout=0.2):
        ...
        logger.info(f"Model size of TiTClf {get_model_size(self)}.")
    ...
    def forward(self, inps, labels = None):
        ...
        if labels is not None:
            ...
            return prob
    def forward_encoder(self, inps, labels = None):
        code_inp, path_inp = inps
        path_inp_shape = path_inp.shape
        code_padding_mask = code_inp.eq(PAD_ID)
        path_padding_mask = path_inp.eq(PAD_ID)
...
def unit_test():
    ...
    dropout = 0.5
    ncls = 4
    model = TiT_Conv1d(ntokens, ncls, emsize, nhead, nhid, nlayers, dropout = dropout)
    model = model.cuda()
    # print(model)
