...
sys.path.append('/home/REMOVE_NAME/workspace/HiT/methodname/model/encoder')
...
from HiTEncoder import ModelEncoder
...
class Model(nn.Module):
    def __init__(self, args, s_vocab, t_vocab):
        super().__init__()
        self.args = args
        s_vocab = args.s_vocab
        t_vocab = args.t_vocab
        self.left_embedding = LeftEmbedding(args, s_vocab)
        self.right_embedding = RightEmbedding(args, t_vocab)
        self.out_right_embedding = nn.Linear(args.hidden, args.concat_hidden)
        self.right_embedding_prob = nn.Linear(args.concat_hidden, len(t_vocab))

        self.decoder_layer = nn.TransformerDecoderLayer(d_model=args.concat_hidden, nhead=args.attn_heads,
                                                        dim_feedforward=args.d_ff_fold * args.concat_hidden,
        ...
        self.encoder = ModelEncoder(args)
        ...
        if self.args.pointer:
            self.query_linear = nn.Linear(
                self.args.concat_hidden, self.args.concat_hidden)
            self.sentinel = nn.Parameter(torch.rand(1, self.args.concat_hidden))
            self.key_linear = nn.Linear(self.args.concat_hidden, self.args.concat_hidden * self.args.subtokens)
            if self.args.activation == 'gelu':
                self.activation = torch.nn.GELU()
            elif self.args.activation == 'relu':
                self.activation = torch.nn.ReLU()
            if self.args.pointer_type == 'add':
                self.additive_attention_W = nn.Linear(self.args.concat_hidden * 2, self.args.concat_hidden)
                self.additive_attention_v = nn.Parameter(torch.rand(self.args.concat_hidden))
    ...
    def encode(self, data):
        ...
        src, seq_src = self.encoder(input_embs=content_, path_ids = path_ids, input_padding_mask = content_mask)
        # (bs, hidden), (max_code_length, bs, hidden)
        memory = seq_src.permute(1, 0, 2)
    ...
    def pointer(self, out, feature, memory, memory_key_padding_mask, content_e, voc_len):
        ...
        if self.args.pointer_type == 'mul':
            pointer_atten = torch.einsum('bth,bsh->bts', pointer_query, pointer_key) / math.sqrt(self.args.concat_hidden)
    ...
    def decode(self, memory, f_source, memory_key_padding_mask, content_e=None, voc_len=None):
        ...
        f_source_ = self.out_right_embedding(f_source_)
        ...
        out = self.softmax(
            self.right_embedding_prob(feature))
