...
class Trainer:
    def __init__(self, args, model, train_data, valid_data, valid_infer_data, test_infer_data, t_vocab, writer_path):
        self.args = args
        ...
        self.optim = AdamW(self.model.parameters(), lr=self.args.lr, eps=self.args.eps)

        self.accu_steps = self.args.accu_batch_size // self.args.batch_size

        if self.args.lr_sche:
            if not self.args.warmup:
                self.scheduler = ReduceLROnPlateau(self.optim, 'max', verbose=True, patience=0, factor=0.8, min_lr=1e-7)
            else:
                self.scheduler = get_linear_schedule_with_warmup(self.optim, num_warmup_steps=len(
                    self.train_data) // self.accu_steps, num_training_steps=len(
                    self.train_data) // self.accu_steps * self.args.epochs)
    ...
    def search(self, memory, memory_key_padding_mask, content_e, voc_len):
        ...
        for _ in range(self.args.max_target_len):
            param = (memory, f_source, memory_key_padding_mask,
                     content_e, voc_len)
            out = self.model.module.decode(
                *param) if self.wrap else self.model.decode(*param)
            if self.unk_shift:
    ...
    def beam_search(self, memory, memory_key_padding_mask, content_e, voc_len):
        ...
        for i in range(self.args.max_target_len):
            ...
            if i == 0:
                ...
                memory = memory.unsqueeze(
                    1).expand(-1, k, -1, -1).contiguous().view(bs * k, memory.shape[1], -1)
                memory_key_padding_mask = memory_key_padding_mask.unsqueeze(1). \
                    expand(-1, k, -1).contiguous().view(bs * k, -1)
                content_e = content_e.unsqueeze(1).expand(-1, k, -1, -1). \
                    contiguous().view(bs * k, content_e.shape[1], content_e.shape[2])
            else:
    ...
    def iteration(self, epoch, data_loader, train=True):
        ...
        for i, data in data_iter:
            ...
            if train:
                ...
                if (i + 1) % self.accu_steps == 0:
                    self.optim.step()
                    self.optim.zero_grad()
                    if self.args.lr_sche and self.args.warmup:
                        self.scheduler.step()
            else:
                self.model.eval()
                with torch.no_grad():
        ...
        if self.args.save and train:
            ...
            torch.save(self.model.state_dict(), os.path.join(save_dir, "{}_{}.pth".format(self.writer_path, epoch)))
    ...
    def predict(self, epoch, test=True):
        ...
        def get_ref_strings(nums):
            infer_file = data_loader.dataset.data[:nums]  # for tiny data
            refs = []
        ...
        with open(predicted_file_name, 'w') as pred_file:
            predict_strings = []
            predict_idx = []
            ref_idx = []
            for i, data in data_iter:
            ...
            write_strings(predict_strings, pred_file)
        ...
        print(
            ...
            flush=True)
        if not test and not self.args.warmup and self.args.lr_sche:
