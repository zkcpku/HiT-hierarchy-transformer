...
from model.transformer_concat import TransformerClf_concat
...
def convert_examples_to_features(code_tokens, label, code_vocab, args, path_tokens=None,path_vocab=None):
    ...
    path_tokens = path_tokens[:args.block_size-2]
    path_tokens = [path_vocab.cls_token]+path_tokens+[path_vocab.sep_token]
    path_ids = path_vocab.convert_tokens_to_ids(path_tokens)
    padding_length = args.block_size - len(path_ids)
...
def train(args, train_dataset, model, code_vocab, path_vocab, pool):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            loss,logits = model([inputs,path_inputs],labels)
...
def main():
    ...
    if args.whole_path_vocab_path:
        path_vocab = pickle.load(open(args.whole_path_vocab_path,'rb'))

    if args.block_size <= 0:
        args.block_size = 256
    
    if args.model_type == 'transformer_concat_wholepath':
        model=TransformerClf_concat([args.seq_vocab_size,args.whole_path_vocab_size], args.cls_num, args.emb_size, args.nhead,
